...
export default function RegisterPage() {
  ...
  const { register, isLoading, error, clearError, token, isInitialized, initialize } = useAuthStore();
  ...
  useEffect(() => {
    clearError();
  }, [name, email, password, monthlyBudget, clearError]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const budget = monthlyBudget ? parseFloat(monthlyBudget) : 5000;
    const success = await register(email, password, name, budget);
  ...
  return (
    ...
    >
      ...
      {/* ── Soft corner orbs ── */}
      <div className="floating-orb animate-orb-drift-1" style={{ width: 420, height: 420, background: '#8b5cf6', top: -120, right: -120 }} />
      <div className="floating-orb animate-orb-drift-2" style={{ width: 320, height: 320, background: '#6366f1', bottom: -80, left: -80 }} />
      <div className="floating-orb animate-orb-drift-3" style={{ width: 260, height: 260, background: '#06b6d4', top: '25%', left: '10%' }} />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.55, duration: 0.4 }}>
              <label className="block text-xs font-medium text-slate-400 mb-2 uppercase tracking-wider">
                Monthly Budget <span className="text-slate-500 normal-case">(optional)</span>
              </label>
              <input
                type="number"
                className="input-glass"
